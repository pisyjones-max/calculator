...
## Про бесплатный тариф (Free) на Render

`render.yaml` настроен на `plan: free` — платить не нужно. Нюансы бесплатного
тарифа, которые стоит знать заранее:

- **Сервис "засыпает" после ~15 минут без запросов** и просыпается ~30-60
  секунд при следующем обращении (первый расчёт после простоя будет с
  задержкой — это нормально, не баг).
- **750 часов/месяц** бесплатно — для одного сервиса, работающего постоянно,
  этого достаточно (в месяце ≈730 часов).
- Диск эфемерный (уже описано в разделе про базу данных ниже) — на free
  тарифе постоянный диск и так недоступен, так что тут ничего не меняется.
- Если 30-60 секунд "прогрева" неприемлемы для реальных клиентов — можно
  позже одной кнопкой в Render Dashboard перейти на платный `Starter`
  (~$7/мес) без пересоздания сервиса, просто сменить план.

На Railway аналогично есть бесплатный стартовый лимит (обычно в виде
$5 кредита в месяц на новых аккаунтах) — уточните актуальные условия в
самом Railway при регистрации, т.к. правила периодически меняются.
